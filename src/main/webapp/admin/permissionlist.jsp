...
<form class="layui-form">
    <div class="layui-form-item" style="margin:15px;height:30px;">
        <div style="height:40px;width:70px;float:left;line-height:40px;">权限名称:</div>
        <input type="text" class="layui-input" style="display:inline-block;width:250px;float:left;" id="keyword"
               name="keyword" value="" lay-verify="" placeholder="请输入权限名称" autocomplete="off">
...
<div id="formData" style="width:350px;display:none;padding-top:15px;">
    <form class="layui-form" id="permissionForm">
        <input type="hidden" name="id" id="showPermissionId"/>
        <div class="layui-form-item">
            <label class="layui-form-label">权限名</label>
            <div class="layui-input-block">
                <input type="text" name="name" id="showPermissionName" required lay-verify="required" value=""
                       class="layui-input"/>
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="url" id="showPermissionUrl" value=""
                       class="layui-input"/>
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <select name="parentId" id="chooseUpdate" lay-verify="required">
                </select>
            </div>
        </div>
...
<div id="addData" style="width:350px;display:none;padding-top:15px;">
    <form class="layui-form" id="addPermissionForm">
        ...
        <div class="layui-form-item">
            <label class="layui-form-label">URL</label>
            <div class="layui-input-block">
                <input type="text" id="addTypeUrl" name="url" value=""
                       class="layui-input"/>
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">上一级</label>
            <div class="layui-input-block">
                <select name="parentId" id="chooseInsert" lay-verify="required">
                </select>
            </div>
        </div>
        <div class="layui-form-item" style="color: darkred; text-align: center">
    ...
    layui.use(['layer', 'table', 'form'], function () {
        ...
        var tableIns = table.render({
            ...
            , cols: [[
                ...
                , {field: 'id', title: '权限编号', width: 120, align: 'center'}
                , {field: 'name', title: '权限名称', width: 300, align: 'center'}
                , {field: 'url', title: '权限URL', width: 300, align: 'center'}
                , {
                    field: 'status', title: '权限状态', width: 112, align: 'center', templet: function (data) {
                        if (data.status === 0) {
        ...
        $("#add").click(function () {
            layer.open({
                ...
                shadeClose: true,
                maxmin: false,
                area: ['400px;', '600px;'],    //窗体大小（宽,高）
                content: $('#addData'),
                success: function (layero, index) {
                    $("#chooseInsert").html("");
                    $.post("/api/permissions/beforeInsert", function (data) {
                        var list = data.data;
                ...
                yes: function (index, layero) {
                    form.on('submit(fromContent)', function (data) {
                        $.post('/api/permissions/insert', $('#addPermissionForm').serialize(), function (res) {
                            if (res.message === "success") {
                                parent.layer.msg('添加权限成功！', {icon: 1, shade: 0.4, time: 1000});
                                // 添加成功后 再次异步请求 渲染表格
                                ...
                                parent.layer.msg('添加权限失败，请重试！', {icon: 5, shade: 0.4, time: 1000});
                            }
                        });
                    })
                }
        ...
        $("#delete").click(function () {
            ...
            if (count > 0) {
                parent.layer.confirm('确定要删除所选权限？', {icon: 3}, function (index) {
                    ...
                    $.ajax({
                        url: '/api/permissions/batchDelete',
                        type: "post",
        ...
        form.on('switch(switchTest)', function (data) {
            ...
            if (flag) {
                $.ajax({
                    type: "post",
                    url: "/api/permissions/changeStatus",
                    data: {"id": id, "status": 2},
                    success: function (data) {
                        if (data.message === "success") {
                            layer.msg('禁用权限成功！', {icon: 1, time: 2000});
                        } else {
                            layer.msg('禁用权限失败，请重试！', {icon: 5, time: 2000});
                        }
                    }
                });
            } else {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.message === "success") {
                            layer.msg("解禁权限成功！", {icon: 1, time: 2000});
                        } else {
                            layer.msg("解禁权限失败，请重试！", {icon: 5, time: 2000});
                        }
                    }
                });
            }
        ...
        table.on('tool(type)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'edit') { //编辑
                layer.open({
                    ...
                    success: function (layero, index) {
                        $("#chooseUpdate").html("");
                        $.post("/api/permissions/beforeUpdate", {"id": data.id}, function (data) {
                            var list = data.data;
                            var parentId = data.message;
                            var str = '<option value="'+parentId+'">保持原上级</option><option value="0">无</option>';
                            for (var i = 0; i < list.length; i++) {
                                str += '<option value="'+list[i].id+'">'+list[i].name+'</option>';
                            }
                        ...
                        form.render();
                        $("#showPermissionId").val(data.id);
                        $("#showPermissionName").val(data.name);
                        $("#showPermissionUrl").val(data.url);
                        body.find('#hidValue').val(index); //将本层的窗口索引传给子页面层的hidValue中
                    },
                    btn: ['修改', '取消'],
                    yes: function (index, layero) {
                        form.on('submit(editFormContent)', function (data) {
                            $.post('/api/permissions/update', $('#permissionForm').serialize(), function (res) {
                                if (res.message === "success") {
                                    parent.layer.msg('编辑权限成功！', {icon: 1, shade: 0.4, time: 1000});
                                    // 编辑成功后 再次异步请求 渲染表格
                                    var word = $("#keyword").val();
                                    tableIns.reload({
                                        where: {
                                    ...
                                    parent.layer.msg('编辑权限失败，请重试！', {icon: 5, shade: 0.4, time: 1000});
                                }
                            });
                        })
                    }
                ...
                layer.confirm('是否删除该权限？', {
                    ...
                    , btn1: function (index, layero) {
                        $.ajax({
                            type: "POST",
                            url: "/api/permissions/deleteLogic",
                            data: {"id": data.id},
                ...
                layer.open({
                    ...
                    success: function (layero, index) {
                        ...
                        $("#leftRoleList").html("");
                        var id = data.id;
                        $.post("/api/permissions/selectAssign",{"id": id},function (data) {
                    ...
                    yes: function (index, layero) {
                        ...
                        $.post("/api/permissions/assign", {"id": id, "roleIds": roleIds}, function (data) {
                            if (data.message === "success") {
