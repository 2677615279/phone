...
<form class="layui-form">
    <div class="layui-form-item" style="margin:15px;height:30px;">
        <div style="height:40px;width:70px;float:left;line-height:40px;">省份名称:</div>
        <input type="text" class="layui-input" style="display:inline-block;width:250px;float:left;" id="keyword"
               name="keyword" value="" lay-verify="" placeholder="请输入省份名称" autocomplete="off">
...
<div id="formData" style="width:350px;display:none;padding-top:15px;">
    <form class="layui-form" id="provinceForm">
        <input type="hidden" name="id" id="showProvinceId"/>
        <div class="layui-form-item">
            <label class="layui-form-label">省名</label>
            <div class="layui-input-block">
                <input type="text" name="name" id="showProvinceName" required lay-verify="required" value=""
                       class="layui-input"/>
            </div>
        </div>
    </form>
...
<div id="addData" style="width:350px;display:none;padding-top:15px;">
    <form class="layui-form" id="addProvinceForm">
        <div class="layui-form-item">
            <label class="layui-form-label">省名</label>
            <div class="layui-input-block">
...
<script type="text/javascript">
    ...
    layui.use(['layer', 'table', 'form'], function () {
        ...
        var tableIns = table.render({
            elem: '#type'
            , id: 'type'
            , url: '/api/provinces/list'
            , method: 'post'
            , loading: false
            , width: 796
            , cols: [[
                {checkbox: true, LAY_CHECKED: false} //其它参数在此省略
                , {field: 'id', title: '省份编号', width: 120, align: 'center'}
                , {field: 'name', title: '省份名称', width: 350, align: 'center'}
                , {
                    field: 'status', title: '省份状态', width: 112, align: 'center', templet: function (data) {
                        if (data.status === 0) {
            ...
            layer.open({
                ...
                yes: function (index, layero) {
                    form.on('submit(fromContent)', function (data) {
                        $.post('/api/provinces/insert', $('#addProvinceForm').serialize(), function (res) {
                            if (res.message === "success") {
                                parent.layer.msg('添加省份成功！', {icon: 1, shade: 0.4, time: 1000});
                                // 添加成功后 再次异步请求 渲染表格
                                ...
                                $("#addProvinceForm")[0].reset();
                                layui.form.render();
                                layer.close(index);
                            } else {
                                parent.layer.msg("添加省份失败，请重试！", {icon: 5, shade: 0.4, time: 1000});
                            }
                        });
                    })
                }
        ...
        $("#delete").click(function () {
            ...
            if (count > 0) {
                parent.layer.confirm('确定要删除所选省份？', {icon: 3}, function (index) {
        ...
        form.on('switch(switchTest)', function (data) {
            ...
            if (flag) {
                $.ajax({
                    type: "post",
                    url: "/api/provinces/changeStatus",
                    data: {"id": id, "status": 2},
                    success: function (data) {
                        if (data.message === "success") {
                            layer.msg('禁用省份成功！', {icon: 1, time: 2000});
                        } else {
                            layer.msg('禁用省份失败，请重试！', {icon: 5, time: 2000});
                        }
                    }
                });
            } else {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.message === "success") {
                            layer.msg("解禁省份成功！", {icon: 1, time: 2000});
                        } else {
                            layer.msg("解禁省份失败，请重试！", {icon: 5, time: 2000});
                        }
                    }
                });
            }
            ...
            if (layEvent === 'edit') { //编辑
                layer.open({
                    ...
                    success: function (layero, index) {
                        ...
                        form.render();
                        $("#showProvinceId").val(data.id);
                        $("#showProvinceName").val(data.name);
                        body.find('#hidValue').val(index); //将本层的窗口索引传给子页面层的hidValue中
                    },
                    btn: ['修改', '取消'],
                    yes: function (index, layero) {
                        form.on('submit(editFormContent)', function (data) {
                            $.post('/api/provinces/update', $('#provinceForm').serialize(), function (res) {
                                if (res.message === "success") {
                                    parent.layer.msg('编辑省份成功！', {icon: 1, shade: 0.4, time: 1000});
                                    // 编辑成功后 再次异步请求 渲染表格
                                    var word = $("#keyword").val();
                                    tableIns.reload({
                                        where: {
                                    ...
                                    parent.layer.msg('编辑省份失败，请重试！', {icon: 5, shade: 0.4, time: 1000});
                                }
                            });
                        })
                    }
                ...
                layer.confirm('是否删除该省份？', {
                    btn: ['确认', '取消'] //可以无限个按钮
                    , btn1: function (index, layero) {
                        $.ajax({
                            type: "POST",
                            url: "/api/provinces/deleteLogic",
                            data: {"id": data.id},
